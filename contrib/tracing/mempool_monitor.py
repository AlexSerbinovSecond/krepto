...
""" Example logging Krepto core mempool events using the mempool:added,
    ...
    def parse_event(event):
        ...
        if type_ == "added":
            return (
                ...
                f" with feerate {data.fee/data.vsize:.2f} kat/vB"
                f" ({data.fee} kat, {data.vsize} vbytes)"
            )
        ...
        if type_ == "replaced":
            return (
                ...
                f" with feerate {data.replaced_fee/data.replaced_vsize:.2f} kat/vB"
                f" received {ts_dt.timestamp()-data.replaced_entry_time:.1f} seconds ago"
                f" ({data.replaced_fee} kat, {data.replaced_vsize} vbytes)"
                f" with {bytes(data.replacement_hash)[::-1].hex()}"
                f" with feerate {data.replacement_fee/data.replacement_vsize:.2f} kat/vB"
                f" ({data.replacement_fee} kat, {data.replacement_vsize} vbytes)"
            )
