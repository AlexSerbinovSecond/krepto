...
The data directory is the default location where the Krepto core files are stored.
...
---------|--------------------
Linux    | `$HOME/.krepto/`
macOS    | `$HOME/Library/Application Support/Krepto/`
Windows  | `%LOCALAPPDATA%\Krepto\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `krepto.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Krepto blocks (dumped in network format, 128 MiB per file)
...
`./`               | `krepto.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `kryptod` or `krepto-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
...
These subdirectories and files are no longer used by Krepto core:
