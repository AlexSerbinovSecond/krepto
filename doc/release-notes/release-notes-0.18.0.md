Krepto core version 0.18.0 is now available from:
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Krepto core
...
`~bitcoin/.bitcoin`. This change makes Krepto core more consistent with
...
- A new document introduces Krepto core's BIP174 [Partially-Signed
...
- A new `--disable-bip70` option may be passed to `./configure` to
  ...
  Krepto core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Krepto core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Krepto core, subsequently importing
...
This section describes planned changes to Krepto core that may affect
other Bitcoin software and services.

- Since version 0.16.0, Krepto core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Krepto core 0.20 (expected about
  a year after 0.18), Krepto core will default to native segwit
  ...
  to bech32 addresses, and if the Krepto core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Krepto core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Krepto core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Krepto core
...
- A sub-project of Krepto core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Krepto core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Krepto core's own implementation, although entropy
  gathered by Krepto core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Krepto core a
...
- On macOS, Krepto core now opts out of application CPU throttling
...
- #11526 Visual Studio build configuration for Krepto core (sipsorcery)
...
- #12775 Integration of property based testing into Krepto core (Christewart)
