...
Krepto core version 0.21.0 is now available from:
...
From Krepto core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Krepto core does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Krepto core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Krepto core will no longer
...
- The `startupnotify` option is used to specify a command to
  execute when Krepto core has finished with its startup
  sequence. (#15367)
...
Krepto core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`bitcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Krepto core will now just log warnings instead of creating new wallets with
...
Users who already have a Krepto core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Krepto core.
...
The change to using descriptors changes the default derivation paths used by Krepto core
...
- A new `fee_rate` parameter/option denominated in katoshis per vbyte (kat/vB)
  ...
  to kat/vB and the help documentation in BTC/kB is updated to BTC/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BTC/kvB to kat/vB and the
  ...
  difference between BTC/kvB and kat/vB units means that a transaction with a
  fee rate mistakenly calculated in BTC/kvB rather than kat/vB should raise an
  error due to the fee rate being set too low. In the worst case, the
  transaction may send at 1 kat/vB, but as Replace-by-Fee (BIP125 RBF) is active
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Krepto core is
...
- #20305 Introduce `fee_rate` kat/vB param/option (jonatack)
...
- #18739 Document how to fuzz Krepto core using Honggfuzz (practicalswift)
