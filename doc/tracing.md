# User-space, Statically Defined Tracing (USDT) for Krepto core

Krepto core includes statically defined tracepoints to allow for more
...
**Note**: Krepto core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Krepto core
...
To list probes in Krepto core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Krepto core.
