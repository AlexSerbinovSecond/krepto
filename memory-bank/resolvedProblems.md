...
## 🔧 Проблема #31: Неправильний URI Префікс в GUI
...
В діалозі "Request payment" GUI клієнта показувався URI з префіксом `bitcoin:` замість `krepto:`, що створювало плутанину для користувачів.

#### Симптоми
- При натисканні кнопки "Receive" в GUI
- В діалозі показувався URI типу: `bitcoin:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa?amount=1.0`
- Замість очікуваного: `krepto:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa?amount=1.0`

### Причина Проблеми

**Основна проблема**: Жорстко закодований префікс `"bitcoin:"` в функціях обробки URI.

#### Проблемні Файли
1. **src/qt/guiutil.cpp**:
   - Функція `parseBitcoinURI()` перевіряла схему `"bitcoin"`
   - Функція `formatBitcoinURI()` використовувала префікс `"bitcoin:"`

2. **Тести**:
   - `src/qt/test/uritests.cpp` - всі тести використовували `bitcoin:` URI
   - `src/qt/test/wallettests.cpp` - перевірка `uri.count("bitcoin:")` в тестах

### Кроки Діагностики

#### 1. Пошук Проблемного Коду
```bash
# Знайдено функції в guiutil.cpp
codebase_search "bitcoin: URI prefix payment request"
```

#### 2. Аналіз Функцій
```cpp
// Проблемна функція parseBitcoinURI
if(!uri.isValid() || uri.scheme() != QString("bitcoin"))
    return false;

// Проблемна функція formatBitcoinURI  
QString ret = QString("bitcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
```

#### 3. Перевірка Тестів
```cpp
// Проблемні тести в uritests.cpp
uri.setUrl(QString("bitcoin:175tWpb8K1S7NmH4Zx6rewF9WQrcZv245W?amount=0.001"));

// Проблемний тест в wallettests.cpp
QCOMPARE(uri.count("bitcoin:"), 2);
```

### Остаточне Рішення

#### 1. Виправлення guiutil.cpp
```cpp
// src/qt/guiutil.cpp - parseBitcoinURI
...
// src/qt/guiutil.cpp - formatBitcoinURI  
...
```cpp
// src/qt/test/uritests.cpp - всі URI замінено на krepto:
...
// src/qt/test/wallettests.cpp - перевірка змінена на krepto:
QCOMPARE(uri.count("krepto:"), 2);
```

#### 3. Перевірка paymentserver.cpp
```cpp
// Вже було правильно налаштовано:
...
# Запуск GUI для тестування
./src/qt/bitcoin-qt -datadir=/Users/serbinov/.krepto &

# Перевірка в GUI:
# 1. Відкрити вкладку "Receive"
# 2. Натиснути "Request payment"  
# 3. Перевірити URI в діалозі - тепер показує "krepto:"
```
...
#### До Виправлення
```
URI: bitcoin:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa?amount=1.0&label=Test
```

#### Після Виправлення  
```
URI: krepto:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa?amount=1.0&label=Test
```

### Файли Змінено

1. **src/qt/guiutil.cpp**:
   - `parseBitcoinURI()` - схема перевірки
   - `formatBitcoinURI()` - префікс перевірки

2. **src/qt/test/uritests.cpp**:
   - Всі тестові URI змінено на krepto:
   - 16 рядків оновлено

3. **src/qt/test/wallettests.cpp**:
   - Перевірка перевірки змінена на krepto:
   - 1 рядок оновлено

### Ключові Уроки

1. **Консистентність Брендингу**: Всі URI повинні використовувати правильний префікс криптовалюти
2. **Тестування GUI**: Важливо перевіряти візуальні елементи після змін
3. **Оновлення Тестів**: При зміні функціональності обов'язково оновлювати відповідні тести
4. **Пошук по Кодовій Базі**: Використовувати semantic search для знаходження всіх місць використання

### Алгоритм для Майбутнього

При зміні URI схеми:

1. **Знайти всі використання**:
   ```bash
   grep -r "bitcoin:" src/qt/
   codebase_search "bitcoin: URI"
   ```

2. **Оновити основні функції**:
   - `parseBitcoinURI()` - схема перевірки
   - `formatBitcoinURI()` - генерація URI

3. **Оновити тести**:
   - Всі тестові URI
   - Перевірки в assert'ах

4. **Перевірити константи**:
   - `BITCOIN_IPC_PREFIX` в paymentserver.cpp
...
5. **Тестування**:
   ...
   make -j8
   ./src/qt/bitcoin-qt &
   # Перевірити GUI діалоги
   ```

### Інструменти та Ресурси

- **Semantic Search**: Для знаходження всіх використань URI
- **GUI Testing**: Ручна перевірка діалогів
- **Unit Tests**: Автоматична перевірка функцій

**Час вирішення**: 1 година  
**Складність**: Середня (потребувала знаходження всіх місць використання)  
**Важливість**: Висока (впливає на користувацький досвід)
