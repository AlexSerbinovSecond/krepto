...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Krepto address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no krepto: URI
    if(!uri.isValid() || uri.scheme() != QString("krepto"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("krepto:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
