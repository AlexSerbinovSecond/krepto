...
const QString BITCOIN_IPC_PREFIX("krepto:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // krepto: URI
        {
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start krepto: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling krepto: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("krepto://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'krepto://' is not a valid URI. Use 'krepto:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // krepto: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Krepto address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
