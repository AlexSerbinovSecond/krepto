// Copyright (c) 2019-present The Krepto core developers
...
struct InputResult {
    ...
    InputResult(A&& in_nsat, B&& in_sat) : nsat(std::forward<A>(in_nsat)), kat(std::forward<B>(in_sat)) {}
};
...
struct Ops {
    ...
    Ops(uint32_t in_count, MaxInt<uint32_t> in_sat, MaxInt<uint32_t> in_dsat) : count(in_count), kat(in_sat), dsat(in_dsat) {};
};
...
struct StackSize {
    const SatInfo kat, dsat;

    constexpr StackSize(SatInfo in_sat, SatInfo in_dsat) noexcept : kat(in_sat), dsat(in_dsat) {};
    constexpr StackSize(SatInfo in_both) noexcept : kat(in_both), dsat(in_both) {};
};
...
struct WitnessSize {
    ...
    WitnessSize(MaxInt<uint32_t> in_sat, MaxInt<uint32_t> in_dsat) : kat(in_sat), dsat(in_dsat) {};
};
...
private:
    internal::Ops CalcOps() const {
        switch (fragment) {
            ...
            case Fragment::AND_V: return {subs[0]->ops.count + subs[1]->ops.count, subs[0]->ops.kat + subs[1]->ops.kat, {}};
            case Fragment::AND_B: {
                const auto count{1 + subs[0]->ops.count + subs[1]->ops.count};
                const auto kat{subs[0]->ops.kat + subs[1]->ops.kat};
                const auto dsat{subs[0]->ops.dsat + subs[1]->ops.dsat};
                return {count, kat, dsat};
            }
            case Fragment::OR_B: {
                const auto count{1 + subs[0]->ops.count + subs[1]->ops.count};
                const auto kat{(subs[0]->ops.kat + subs[1]->ops.dsat) | (subs[1]->ops.kat + subs[0]->ops.dsat)};
                const auto dsat{subs[0]->ops.dsat + subs[1]->ops.dsat};
                return {count, kat, dsat};
            }
            case Fragment::OR_D: {
                const auto count{3 + subs[0]->ops.count + subs[1]->ops.count};
                const auto kat{subs[0]->ops.kat | (subs[1]->ops.kat + subs[0]->ops.dsat)};
                const auto dsat{subs[0]->ops.dsat + subs[1]->ops.dsat};
                return {count, kat, dsat};
            }
            case Fragment::OR_C: {
            ...
            case Fragment::ANDOR: {
                ...
                const auto kat{(subs[1]->ops.kat + subs[0]->ops.kat) | (subs[0]->ops.dsat + subs[2]->ops.kat)};
            ...
            case Fragment::WRAP_C:
            case Fragment::WRAP_N: return {1 + subs[0]->ops.count, subs[0]->ops.kat, subs[0]->ops.dsat};
            case Fragment::WRAP_A: return {2 + subs[0]->ops.count, subs[0]->ops.kat, subs[0]->ops.dsat};
            case Fragment::WRAP_D: return {3 + subs[0]->ops.count, subs[0]->ops.kat, 0};
            case Fragment::WRAP_J: return {4 + subs[0]->ops.count, subs[0]->ops.kat, 0};
            case Fragment::WRAP_V: return {subs[0]->ops.count + (subs[0]->GetType() << "x"_mst), subs[0]->ops.kat, {}};
            case Fragment::THRESH: {
                uint32_t count = 0;
                ...
                for (const auto& sub : subs) {
                    ...
                    for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back((sats[j] + sub->ops.dsat) | (sats[j - 1] + sub->ops.kat));
                    next_sats.push_back(sats[sats.size() - 1] + sub->ops.kat);
    ...
    internal::StackSize CalcStackSize() const {
        ...
        switch (fragment) {
            ...
            case Fragment::ANDOR: {
                ...
                return {
                    (x.kat + SatInfo::If() + y.kat) | (x.dsat + SatInfo::If() + z.kat),
            ...
            case Fragment::AND_B: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {x.kat + y.kat + SatInfo::BinaryOp(), x.dsat + y.dsat + SatInfo::BinaryOp()};
            }
            case Fragment::OR_B: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {
                    ((x.kat + y.dsat) | (x.dsat + y.kat)) + SatInfo::BinaryOp(),
            ...
            case Fragment::OR_C: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {(x.kat + SatInfo::If()) | (x.dsat + SatInfo::If() + y.kat), {}};
            }
            case Fragment::OR_D: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {
                    (x.kat + SatInfo::OP_IFDUP(true) + SatInfo::If()) | (x.dsat + SatInfo::OP_IFDUP(false) + SatInfo::If() + y.kat),
            ...
            case Fragment::OR_I: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {SatInfo::If() + (x.kat | y.kat), SatInfo::If() + (x.dsat | y.dsat)};
            }
            ...
            case Fragment::WRAP_C: return {
                subs[0]->ss.kat + SatInfo::OP_CHECKSIG(),
                subs[0]->ss.dsat + SatInfo::OP_CHECKSIG()
            };
            case Fragment::WRAP_D: return {
                SatInfo::OP_DUP() + SatInfo::If() + subs[0]->ss.kat,
                SatInfo::OP_DUP() + SatInfo::If()
            };
            case Fragment::WRAP_V: return {subs[0]->ss.kat + SatInfo::OP_VERIFY(), {}};
            case Fragment::WRAP_J: return {
                SatInfo::OP_SIZE() + SatInfo::OP_0NOTEQUAL() + SatInfo::If() + subs[0]->ss.kat,
            ...
            case Fragment::THRESH: {
                ...
                for (size_t i = 0; i < subs.size(); ++i) {
                    ...
                    for (size_t j = 1; j < sats.size(); ++j) {
                        next_sats.push_back(((sats[j] + subs[i]->ss.dsat) | (sats[j - 1] + subs[i]->ss.kat)) + add);
                    }
                    // Finally construct next_sats[i+1].
                    next_sats.push_back(sats[sats.size() - 1] + subs[i]->ss.kat + add);
                    // Switch over.
                    sats = std::move(next_sats);
                }
    ...
    internal::WitnessSize CalcWitnessSize() const {
        ...
        switch (fragment) {
            ...
            case Fragment::ANDOR: {
                const auto kat{(subs[0]->ws.kat + subs[1]->ws.kat) | (subs[0]->ws.dsat + subs[2]->ws.kat)};
                const auto dsat{subs[0]->ws.dsat + subs[2]->ws.dsat};
                return {kat, dsat};
            }
            case Fragment::AND_V: return {subs[0]->ws.kat + subs[1]->ws.kat, {}};
            case Fragment::AND_B: return {subs[0]->ws.kat + subs[1]->ws.kat, subs[0]->ws.dsat + subs[1]->ws.dsat};
            case Fragment::OR_B: {
                const auto kat{(subs[0]->ws.dsat + subs[1]->ws.kat) | (subs[0]->ws.kat + subs[1]->ws.dsat)};
                const auto dsat{subs[0]->ws.dsat + subs[1]->ws.dsat};
                return {kat, dsat};
            }
            case Fragment::OR_C: return {subs[0]->ws.kat | (subs[0]->ws.dsat + subs[1]->ws.kat), {}};
            case Fragment::OR_D: return {subs[0]->ws.kat | (subs[0]->ws.dsat + subs[1]->ws.kat), subs[0]->ws.dsat + subs[1]->ws.dsat};
            case Fragment::OR_I: return {(subs[0]->ws.kat + 1 + 1) | (subs[1]->ws.kat + 1), (subs[0]->ws.dsat + 1 + 1) | (subs[1]->ws.dsat + 1)};
            ...
            case Fragment::WRAP_D: return {1 + 1 + subs[0]->ws.kat, 1};
            case Fragment::WRAP_V: return {subs[0]->ws.kat, {}};
            case Fragment::WRAP_J: return {subs[0]->ws.kat, 1};
            case Fragment::THRESH: {
                auto sats = Vector(internal::MaxInt<uint32_t>(0));
                for (const auto& sub : subs) {
                    auto next_sats = Vector(sats[0] + sub->ws.dsat);
                    for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back((sats[j] + sub->ws.dsat) | (sats[j - 1] + sub->ws.kat));
                    next_sats.push_back(sats[sats.size() - 1] + sub->ws.kat);
                    sats = std::move(next_sats);
                }
                assert(k <= sats.size());
    ...
    internal::InputResult ProduceInput(const Ctx& ctx) const {
            ...
            switch (node.fragment) {
                ...
                case Fragment::MULTI_A: {
                    ...
                    for (size_t i = 0; i < node.keys.size(); ++i) {
                        ...
                        auto kat = InputStack(std::move(sig)).SetWithSig().SetAvailable(avail);
                        ...
                        for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back((sats[j] + ZERO) | (std::move(sats[j - 1]) + kat));
                        next_sats.push_back(std::move(sats[sats.size() - 1]) + std::move(kat));
                        // Switch over.
                        sats = std::move(next_sats);
                    }
                ...
                case Fragment::MULTI: {
                    ...
                    for (size_t i = 0; i < node.keys.size(); ++i) {
                        ...
                        for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back(sats[j] | (std::move(sats[j - 1]) + kat));
                        next_sats.push_back(std::move(sats[sats.size() - 1]) + std::move(kat));
                        // Switch over.
                        sats = std::move(next_sats);
                    }
                ...
                case Fragment::THRESH: {
                    ...
                    for (size_t i = 0; i < subres.size(); ++i) {
                        ...
                        // so far. next_sats[j] is either sats[j] + res.nsat (reusing j earlier satisfactions) or sats[j-1] + res.kat
                        ...
                        for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back((sats[j] + res.nsat) | (std::move(sats[j - 1]) + res.kat));
                        next_sats.push_back(std::move(sats[sats.size() - 1]) + std::move(res.kat));
                        // Switch over.
                        sats = std::move(next_sats);
                    }
                    // At this point, sats[k].kat is the best satisfaction for the overall thresh() node. The best dissatisfaction
                ...
                case Fragment::AND_V: {
                    ...
                    return {(y.nsat + x.kat).SetNonCanon(), y.kat + x.kat};
                }
                case Fragment::AND_B: {
                    auto& x = subres[0], &y = subres[1];
                    ...
                    return {(y.nsat + x.nsat) | (y.kat + x.nsat).SetMalleable().SetNonCanon() | (y.nsat + x.kat).SetMalleable().SetNonCanon(), y.kat + x.kat};
                }
                case Fragment::OR_B: {
                    auto& x = subres[0], &z = subres[1];
                    // The (kat(Z) kat(X)) solution is overcomplete (attacker can change either into dsat).
                    return {z.nsat + x.nsat, (z.nsat + x.kat) | (z.kat + x.nsat) | (z.kat + x.kat).SetMalleable().SetNonCanon()};
                }
                case Fragment::OR_C: {
                    auto& x = subres[0], &z = subres[1];
                    return {INVALID, std::move(x.kat) | (z.kat + x.nsat)};
                }
                case Fragment::OR_D: {
                    auto& x = subres[0], &z = subres[1];
                    return {z.nsat + x.nsat, std::move(x.kat) | (z.kat + x.nsat)};
                }
                case Fragment::OR_I: {
                    auto& x = subres[0], &z = subres[1];
                    return {(x.nsat + ONE) | (z.nsat + ZERO), (x.kat + ONE) | (z.kat + ZERO)};
                }
                case Fragment::ANDOR: {
                    auto& x = subres[0], &y = subres[1], &z = subres[2];
                    return {(y.nsat + x.kat).SetNonCanon() | (z.nsat + x.nsat), (y.kat + x.kat) | (z.kat + x.nsat)};
                }
                case Fragment::WRAP_A:
                case Fragment::WRAP_S:
                case Fragment::WRAP_C:
                ...
                case Fragment::WRAP_J: {
                    ...
                    return {InputStack(ZERO).SetMalleable(x.nsat.available != Availability::NO && !x.nsat.has_sig), std::move(x.kat)};
                }
                case Fragment::WRAP_V: {
                    auto &x = subres[0];
                    return {INVALID, std::move(x.kat)};
                }
        ...
        auto tester = [&helper](const Node& node, Span<InputResult> subres) -> InputResult {
            ...
            if (node.GetType() << "z"_mst && ret.kat.available != Availability::NO) assert(ret.kat.stack.size() == 0);
            ...
            if (node.GetType() << "o"_mst && ret.kat.available != Availability::NO) assert(ret.kat.stack.size() == 1);
            ...
            if (node.GetType() << "n"_mst && ret.kat.available != Availability::NO) assert(ret.kat.stack.size() >= 1);
            if (node.GetType() << "n"_mst && ret.nsat.available != Availability::NO) assert(ret.nsat.stack.size() >= 1);
            if (node.GetType() << "n"_mst && ret.kat.available != Availability::NO) assert(!ret.kat.stack.back().empty());
            ...
            if (node.GetType() << "s"_mst && ret.kat.available != Availability::NO) assert(ret.kat.has_sig);
            ...
            if (node.GetType() << "m"_mst && ret.kat.available != Availability::NO) assert(!ret.kat.malleable);

            // If a non-malleable satisfaction exists, it must be canonical.
            if (ret.kat.available != Availability::NO && !ret.kat.malleable) assert(!ret.kat.non_canon);
...
public:
    ...
    std::optional<uint32_t> GetOps() const {
        if (!ops.kat.valid) return {};
        return ops.count + ops.kat.value;
    }
    ...
    std::optional<uint32_t> GetStackSize() const {
        if (!ss.kat.valid) return {};
        return ss.kat.netdiff + static_cast<int32_t>(IsBKW());
    }

    //! Return the maximum size of the stack during execution of this script.
    std::optional<uint32_t> GetExecStackSize() const {
        if (!ss.kat.valid) return {};
        return ss.kat.exec + static_cast<int32_t>(IsBKW());
    }
    ...
    Availability Satisfy(const Ctx& ctx, std::vector<std::vector<unsigned char>>& stack, bool nonmalleable = true) const {
        ...
        if (nonmalleable && (ret.kat.malleable || !ret.kat.has_sig)) return Availability::NO;
        stack = std::move(ret.kat.stack);
        return ret.kat.available;
    }
