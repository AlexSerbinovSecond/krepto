...
DEFAULT_BLOCK_MIN_TX_FEE = 1000  # default `-blockmintxfee` setting [kat/kvB]
...
class MiningTest(BitcoinTestFramework):
    ...
    def test_blockmintxfee_parameter(self):
        ...
        # test default (no parameter), zero and a bunch of arbitrary blockmintxfee rates [kat/kvB]
        for blockmintxfee_sat_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 0, 50, 100, 500, 2500, 5000, 21000, 333333, 2500000):
            ...
            if blockmintxfee_sat_kvb == DEFAULT_BLOCK_MIN_TX_FEE:
                self.log.info(f"-> Default -blockmintxfee setting ({blockmintxfee_sat_kvb} kat/kvB)...")
            else:
                blockmintxfee_parameter = f"-blockmintxfee={blockmintxfee_btc_kvb:.8f}"
                self.log.info(f"-> Test {blockmintxfee_parameter} ({blockmintxfee_sat_kvb} kat/kvB)...")
            ...
            if blockmintxfee_btc_kvb > 0:
                lowerfee_btc_kvb = blockmintxfee_btc_kvb - Decimal(10)/COIN  # 0.01 kat/vbyte lower
