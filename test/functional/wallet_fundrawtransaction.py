...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate kat/vB and feeRate BTC/kvB)")
        node = self.nodes[3]
        ...
        # With no arguments passed, expect fee of 141 katoshis.
        ...
        self.log.info("Test min fee rate checks are bypassed with fundrawtxn, e.g. a fee_rate under 1 kat/vB is allowed")
        node.fundrawtransaction(rawtx, fee_rate=0.999, add_inputs=True)
        node.fundrawtransaction(rawtx, feeRate=0.00000999, add_inputs=True)

        self.log.info("- raises RPC error if both feeRate and fee_rate are passed")
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (kat/vB) and feeRate (BTC/kvB)",
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with fee_rate (kat/vB)
        btc_kvb_to_sat_vb = 100000  # (1e5)
        result = [self.nodes[3].fundrawtransaction(rawtx),  # uses self.min_relay_tx_fee (set by settxfee)
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[]),  # empty subtraction list
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[0]),  # uses self.min_relay_tx_fee (set by settxfee)
        ...
        # katoshis more, than outputs 2 and 3.
    ...
    def test_feerate_rounding(self):
        ...
        # At a feerate of 1.85 kat/vb, the input will need a fee of 125.8 sats and the rest 77.7 sats
